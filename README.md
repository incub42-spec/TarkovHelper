...
Статус «новое!» не значит ничего: это метка непросмотренного изменения. Она
висит и на только что выданном задании, и на только что **завершённом**,
которое игрок ещё не открывал. Поэтому такие строки программа узнаёт (порядок,
раздел, имя), но статус из них не выводит вовсе.

Порядок квестов в списке программа тоже берёт из кадра: игра сортирует их
по-своему — не по алфавиту, не по уровню и не по порядку в дампе, а по тому,
когда квест был выдан в этом профиле. Из данных такое не вывести, зато видно
на экране. Что ещё не сканировали, идёт следом по требуемому уровню.

Во втором режиме отображения (кнопка справа от галочек) игра раскладывает
задания по разделам: уровни лояльности торговца и «Ключевые». Сканирование
такого экрана запоминает разделы, а галочка **«Группировать по разделам, как
в игре»** во вкладке «Квесты» раскладывает список так же. Уровень лояльности
есть и в базе, но только у 110 квестов из 514 — у остальных он берётся с
экрана; что не сканировали и в базе не указано, собирается в отдельную группу,
чтобы не выдумывать раздел.

Квест со статусом «активно!» остаётся доступным, даже если по базе он игроку
не положен. Требования в дампе отстают от игры: у «Поставщика» там 36 уровень,
а Прапор выдаёт его на 35-м. Увиденное на экране важнее.
...
**Shift+F11 — конец обхода списка.** Каждый обычный F11 копит то, что попалось
в кадре, а Shift+F11 говорит «список пройден до конца». После этого квесты,
которых торговец не показал, помечаются как **не выданные** и уходят из списка
доступных.

Это не отметка «сдан»: с патча 1.1.0 задания приходят пачками по два-четыре, и
невыданные в списке не видны вовсе — даже с галочкой «Заблокированные».
Поэтому программа не гадает, сдан квест или просто не выдан, а записывает то,
что видела: торговец его пока не предлагает. Как только квест появится в кадре,
признак снимется сам.

Так список сходится с игрой один в один, а история сдачи остаётся честной —
её пополняет только сканирование с галочкой «Завершенные».

То же можно сделать руками: выделить строки в списке (можно несколько) и
выбрать в контекстном меню **«Торговец их не предлагает»**. Иногда так проще —
список торговца перед глазами, и сразу видно, чего в нём нет. «Вернуть в
доступные» отменяет пометку.

### Облачное распознавание (необязательно)

Встроенный движок Windows на коротких русских названиях ошибается дорого:
«БАДы» он читает как «6AAbl» — целиком латиницей, и связать такую строку с
базой уже нечем. Если это мешает, во вкладке «Настройки» можно включить
**Yandex Vision OCR**: нужны API-ключ и идентификатор каталога (Yandex Cloud →
сервисный аккаунт с ролью `ai.vision.user`, там же API-ключ).

Включается только для сканирования списков квестов (F11): снимок этой области
уходит в облако. Подсказки по предметам (F9) он не затрагивает — там важнее
мгновенный ответ, а не точность на кириллице. Если облако не ответило, кадр
читает встроенный движок, и сканирование просто продолжает работать — но
подсказка об этом скажет прямо, чтобы «выключено» и «ответило ошибкой» не
выглядели одинаково.

Что важно знать заранее:

* нужен **платёжный аккаунт** — без него ключ создастся, настройки скажут
  «готово», а каждый запрос будет отклонён;
* похожих идентификаторов там четыре, и в запрос идёт только один. В «API-ключ»
  вставляется **секретное значение** ключа (длинная строка, показывается один
  раз при создании), в «Каталог» — идентификатор **каталога** вида `b1g…` из
  адреса консоли после `/folders/` — или из таблицы «Каталоги» в информации об
  облаке. Идентификатор самого **облака** тоже начинается с `b1g…`, но не
  подходит: каталог лежит внутри облака, и ресурсы живут в нём. Всё, что
  начинается с `aje…`, не подходит тем более: так выглядят идентификатор самого
  ключа и сервисный аккаунт. Яндекс на
  путаницу отвечает внятно и сам называет верное значение — текст ошибки виден
  в настройках;
* ключ лучше делать **бессрочным**: иначе через месяц всё встанет, а причину
  придётся искать;
* один кадр стоит около 13 копеек, синхронных запросов разрешён один в
  секунду — программа сама держит этот интервал.

### Сводка по локации (F8)

Игра пишет в свой журнал, в какую локацию вы зашли — строкой вида
`[Transit] … Locations:factory4_day`. Программа читает её и по нажатию **F8**
показывает сводку именно по этой карте:

* активные задания, которые здесь делаются;
* что для них собрать и сколько ещё не хватает (с пометкой FIR);
* **какие ключи взять** — их забывают чаще всего, а узнать об этом уже в
  рейде значит потерять вылазку.

Конца рейда в журнале нет, поэтому «текущей» считается последняя загруженная
локация. Это удобно: сводка работает и в меню, когда к вылазке только
готовятся.
...
- **Квесты** — список всех квестов с отметкой выполнения (поиск, фильтр),
  колонками фракции и локации. Локацию можно выбрать в выпадающем списке над
  таблицей: видно, что реально закрыть за один рейд. Отдельный пункт «Любая
  локация» — задания без привязки к карте, их половина. Выполненные открываются кнопкой «Выполненные…» в отдельном
