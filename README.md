...
- **Цепочки квестов** — у каждого квеста виден статус: «доступен» (все
  предыдущие квесты цепочки сданы и уровень позволяет), «закрыт» или
  «выполнен». Фильтр показывает только доступные — это и есть список того,
  чем можно заняться прямо сейчас. Предметы для закрытых квестов помечаются
  «позже» в списке «Что собирать» и приглушаются в подсказке оверлея, как и
  предметы для дальних уровней убежища.
